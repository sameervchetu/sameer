...
require_once($CFG->dirroot . '/local/sop/uploadcourse/classes/step1_form.php');
require_once($CFG->dirroot . '/local/sop/uploadcourse/classes/step2_form.php');
...
$returnurl = new moodle_url('/local/sop/uploadcourse/index.php');

if (empty($importid)) {
    $mform1 = new sop_tool_uploadcourse_step1_form();
...
$mform2 = new sop_tool_uploadcourse_step2_form(null, array('contextid' => $context->id, 'columns' => $cir->get_columns(),
    'data' => $data));
...
if ($form2data = $mform2->is_cancelled()) {
    ...
    $processor = new sop_tool_uploadcourse_processor($cir, $options, $defaults);

    echo $OUTPUT->header();
    if (isset($form2data->showpreview)) {
        echo $OUTPUT->heading(get_string('uploadcoursespreview', 'tool_uploadcourse'));
        $processor->preview($previewrows, new sop_tool_uploadcourse_tracker(tool_uploadcourse_tracker::OUTPUT_HTML));
        $mform2->display();
    } else {
        echo $OUTPUT->heading(get_string('uploadcoursesresult', 'tool_uploadcourse'));
        $processor->execute(new sop_tool_uploadcourse_tracker(tool_uploadcourse_tracker::OUTPUT_HTML));
    ...
    $processor = new sop_tool_uploadcourse_processor($cir, $form1data->options, array());
    echo $OUTPUT->header();
    echo $OUTPUT->heading(get_string('uploadcoursespreview', 'tool_uploadcourse'));
    $processor->preview($previewrows, new sop_tool_uploadcourse_tracker(tool_uploadcourse_tracker::OUTPUT_HTML));
    $mform2->display();
