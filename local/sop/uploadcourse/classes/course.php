 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class sop_tool_uploadcourse_course {
    ...
    public function __construct($mode, $updatemode, $rawdata, $defaults = array(), $importoptions = array()) {
        ...
        if ($mode !== sop_tool_uploadcourse_processor::MODE_CREATE_NEW &&
                $mode !== sop_tool_uploadcourse_processor::MODE_CREATE_ALL &&
                $mode !== sop_tool_uploadcourse_processor::MODE_CREATE_OR_UPDATE &&
                $mode !== sop_tool_uploadcourse_processor::MODE_UPDATE_ONLY) {
            throw new coding_exception('Incorrect mode.');
        } else if ($updatemode !== sop_tool_uploadcourse_processor::UPDATE_NOTHING &&
                $updatemode !== sop_tool_uploadcourse_processor::UPDATE_ALL_WITH_DATA_ONLY &&
                $updatemode !== sop_tool_uploadcourse_processor::UPDATE_ALL_WITH_DATA_OR_DEFAUTLS &&
                $updatemode !== sop_tool_uploadcourse_processor::UPDATE_MISSING_WITH_DATA_OR_DEFAUTLS) {
    ...
    public function can_create() {
        return in_array($this->mode, array(sop_tool_uploadcourse_processor::MODE_CREATE_ALL,
            sop_tool_uploadcourse_processor::MODE_CREATE_NEW,
            sop_tool_uploadcourse_processor::MODE_CREATE_OR_UPDATE)
        );
    }
    ...
    public function can_update() {
        return in_array($this->mode,
                array(
                    sop_tool_uploadcourse_processor::MODE_UPDATE_ONLY,
                    sop_tool_uploadcourse_processor::MODE_CREATE_OR_UPDATE)
    ...
    public function can_use_defaults() {
        return in_array($this->updatemode, array(sop_tool_uploadcourse_processor::UPDATE_MISSING_WITH_DATA_OR_DEFAUTLS,
            sop_tool_uploadcourse_processor::UPDATE_ALL_WITH_DATA_OR_DEFAUTLS));
    }
    ...
    protected function get_restore_content_dir() {
        ...
        $errors = array();
        $dir = sop_tool_uploadcourse_helper::get_restore_content_dir($backupfile, $shortname, $errors);
        if (!empty($errors)) {
    ...
    public function prepare() {
        ...
        if ($exists) {
            if ($this->mode === sop_tool_uploadcourse_processor::MODE_CREATE_NEW) {
        ...
        $errors = array();
        $catid = sop_tool_uploadcourse_helper::resolve_category($this->rawdata, $errors);
        if (empty($errors)) {
        ...
        if (!$exists || $mode === sop_tool_uploadcourse_processor::MODE_CREATE_ALL) {
        ...
        if (empty($this->shortname) && !is_numeric($this->shortname)) {
            if (empty($this->importoptions['shortnametemplate'])) {
                ...
                $newshortname = sop_tool_uploadcourse_helper::generate_shortname($coursedata,
        ...
        if ($exists && $mode === sop_tool_uploadcourse_processor::MODE_CREATE_ALL) {
            ...
            $this->shortname = sop_tool_uploadcourse_helper::increment_shortname($this->shortname);
            $exists = false;
            if ($this->shortname != $original) {
                ...
                if (isset($coursedata['idnumber'])) {
                    ...
                    $coursedata['idnumber'] = sop_tool_uploadcourse_helper::increment_idnumber($coursedata['idnumber']);
        ...
        switch ($mode) {
            case sop_tool_uploadcourse_processor::MODE_CREATE_NEW:
            case sop_tool_uploadcourse_processor::MODE_CREATE_ALL:
                if ($exists) {
                    $this->error('courseexistsanduploadnotallowed',
                        new lang_string('courseexistsanduploadnotallowed', 'tool_uploadcourse'));
                    return false;
                ...
                break;
            case sop_tool_uploadcourse_processor::MODE_UPDATE_ONLY:
                if (!$exists) {
                    $this->error('coursedoesnotexistandcreatenotallowed',
                        new lang_string('coursedoesnotexistandcreatenotallowed', 'tool_uploadcourse'));
                    return false;
                ...
                // No break!
            case sop_tool_uploadcourse_processor::MODE_CREATE_OR_UPDATE:
                if ($exists) {
                    if ($updatemode === sop_tool_uploadcourse_processor::UPDATE_NOTHING) {
        ...
        // Get final data.
        if ($exists) {
            $missingonly = ($updatemode === sop_tool_uploadcourse_processor::UPDATE_MISSING_WITH_DATA_OR_DEFAUTLS);
        ...
        // Add role renaming.
        $errors = array();
        $rolenames = sop_tool_uploadcourse_helper::get_role_names($this->rawdata, $errors);
        if (!empty($errors)) {
            foreach ($errors as $key => $message) {
                $this->error($key, $message);
            }
        ...
        // Some validation.
        if (!empty($coursedata['format']) && !in_array($coursedata['format'], sop_tool_uploadcourse_helper::get_course_formats())) {
        ...
        $this->enrolmentdata = sop_tool_uploadcourse_helper::get_enrolment_data($this->rawdata);
    ...
    protected function process_enrolment_data($course) {
        ...
        $enrolmentplugins = sop_tool_uploadcourse_helper::get_enrolment_plugins();
        ...
        foreach ($enrolmentdata as $enrolmethod => $method) {
            ...
            if (!empty($instance) && $todelete) {
                ...
                if (isset($method['role'])) {
                    $roleids = sop_tool_uploadcourse_helper::get_role_ids();
    ...
    protected function reset($course) {
        ...
        // Add roles.
        $roles = sop_tool_uploadcourse_helper::get_role_ids();
